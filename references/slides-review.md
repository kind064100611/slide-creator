...
Canonical materialization alone does not establish template fitness. A structurally valid generic cards, list, or summary template still fails when it flattens a reference table, metric hierarchy, grouped peer grid, process relationship, branch, or connector topology. A different template is acceptable only when the current artifacts retain the same source-owned semantic roles, peer cardinality, table dimensions, relationship edges, emphasis order, and readable visual density. Treat a template swap after a materialization error as suspect evidence: it must prove source-structure preservation, not merely that the replacement template is canonical and convertible.
...
- Treat the complete authored emphasized phrase as the semantic emphasis unit. Replacing it with a calculated shorthand, a normalized currency form, or one embedded date/number token changes the message hierarchy even when all original words remain elsewhere; this is at least **Major**. A page-level measure may not be copied into or substituted for a peer-card highlight.
